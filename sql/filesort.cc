...
/**
  ...
  @param
    packed_format         TRUE:  using packing for sort keys and addon fields
                          FALSE: no packing for sortkeys and addon fields
  ...
  if ((count= MY_MIN(buffpek->max_keys(),buffpek->rowcount())))
  {
    ...
    if (packed_format)
    {
      ...
      */
      ...
      uint ix= 0;
      const bool using_packed_sortkeys= param->using_packed_sortkeys();
      const bool using_packed_addons= param->using_packed_addons();

      uint size_of_addon_length= using_packed_addons ?
                                 Addon_fields::size_of_length_field : 0;

      uint size_of_sort_length= using_packed_sortkeys ?
      ...
      for (; ix < count; ++ix)
      {
        if (record + size_of_sort_length > buffpek->buffer_end())
          break;
        uint sort_length=  using_packed_sortkeys ?
        ...
        uint res_length= using_packed_addons ?
                         Addon_fields::read_addon_length(plen):
                         param->res_length;
...
int
Type_handler_string_result::compare_packed_sort_keys(uchar *a, size_t *a_len,
...
*/
int
Type_handler::compare_packed_sort_keys(uchar *a, size_t *a_len,
                                  uchar *b, size_t *b_len,
                                  const SORT_FIELD *sortorder)const
{
                        ...
                        unsigned char **a_ptr, unsigned char **b_ptr)
{
       ...
       sort_field != sort_keys->end(); sort_field++)
  {
    if (sort_field->field)
    {
      retval= sort_field->field->compare_packed_sort_keys(a, &a_len,
                                                          b, &b_len,
                                                          sort_field);
    }
    else
    {
      ...
      retval= item->type_handler()->compare_packed_sort_keys(a, &a_len,
                                                             b, &b_len,
                                                             sort_field);
    }
...
static uint make_packed_sortkey(Sort_param *param, uchar *to)
{
  ...
  /*
    Store the length of the sort key
  */
